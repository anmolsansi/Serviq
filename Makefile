...
.PHONY: dependency-lock-check setup dev test lint typecheck security e2e load-test down

dependency-lock-check:
	@echo "Checking committed Python dependency locks."
	cd services/api && uv lock --check
	cd services/worker && uv lock --check
	cd services/llm-gateway && uv lock --check

setup: dependency-lock-check
...
security: dependency-lock-check
	...
	cd services/api && uv export --frozen --no-dev --format requirements-txt --output-file /tmp/serviq-api-requirements.txt && uvx --python 3.14 --from pip-audit==2.10.1 pip-audit -r /tmp/serviq-api-requirements.txt
	cd services/worker && uv export --frozen --no-dev --format requirements-txt --output-file /tmp/serviq-worker-requirements.txt && uvx --python 3.14 --from pip-audit==2.10.1 pip-audit -r /tmp/serviq-worker-requirements.txt
	cd services/llm-gateway && uv export --frozen --no-dev --format requirements-txt --output-file /tmp/serviq-llm-gateway-requirements.txt && uvx --python 3.14 --from pip-audit==2.10.1 pip-audit -r /tmp/serviq-llm-gateway-requirements.txt
