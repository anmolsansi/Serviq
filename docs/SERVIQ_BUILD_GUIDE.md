...
The running knowledge worker publishes durable outbox events, consumes URL/file
sync work, and continuously reconciles due failed-upload cleanup obligations. It
stores fetched content and creates a document plus a durable parse event. The source
remains `syncing`: no parse consumer persists normalized output, invokes the chunker,
indexes vectors, or makes the source ready. PDF/Markdown/text/HTML normalization and
heading-aware chunking are implemented pure libraries. Sitemap registration exists,
but its sync currently fails as unsupported.
...
The default worker runs the outbox publisher, the knowledge-sync consumer, and the
bounded durable upload-cleanup scheduler. It still stops at the durable parse
handoff; the transformation libraries are not yet called by a parse/index pipeline.
Local Compose and CI/security infrastructure exist.
...
Continue from the existing foundation. The current implementation branch resolves
the proven async-dependency, private-gateway validation, and automatic failed-upload
cleanup-caller defects. GitHub #205, the session/principal contract, pre-spool
multipart admission, URL raw-object accounting, and parse persistence remain open
work. Complete the ingestion/index handoff before V1.3.13. V1.3.12 can use
ADR-027's 1536 dimensions but still needs an exact vector index/operator and safe
migration contract.
...
**Implemented on integration branch:** `ope304`  
**Linear ticket:** `OPE-304`

## What problem this ticket solves

Before OPE-304, Serviq had commits, branches, pull requests, CI, and repository governance, but it did not have an official public version history. A reader could see what changed over time, but there was no permanent answer to a simple question such as:

> "Which exact commit is Serviq version 0.1.0?"

OPE-304 adds that missing release layer with Git tags, GitHub Releases, Semantic Versioning, generated release notes, release-impact metadata on pull requests, and repeatable release automation. It changes repository/release governance only. It does not change Serviq runtime APIs, database contracts, authentication, events, or customer workflows.

## Generated release notes

`.github/release.yml` tells GitHub how merged pull requests should be grouped when release notes are generated. The categories cover breaking changes, security, features, bug fixes, performance, infrastructure, testing, dependencies, documentation, refactors, and a final catch-all for changes that do not match a more specific category.

Pull requests marked `release:skip` are excluded from generated release notes. Existing default GitHub labels such as `bug`, `documentation`, and `enhancement` remain valid and are not removed.

## Semantic Versioning

Serviq release tags use a leading `v`:

```text
vMAJOR.MINOR.PATCH
```

Development releases can add a prerelease suffix:
...
The release workflow validates this format before any release command runs. Major, minor, and patch numbers cannot use leading zeroes. Numeric prerelease identifiers cannot use leading zeroes either, so `v0.2.0-alpha.1` is valid while `v0.2.0-alpha.01` is rejected. Build metadata such as `+build.7` is intentionally outside the OPE-304 release format.

## The three release paths

The permanent workflow is `.github/workflows/release.yml`.

It supports three release paths:

1. the one-time foundation prerelease bootstrap;
2. a manual release started from GitHub Actions;
3. a release triggered by an externally pushed semantic-version tag.

The manual path checks out `main`, validates the requested version, runs the repository quality gates, verifies that the tested commit is still contained in `main`, and then creates the tag and GitHub Release at that tested commit.

The tag-driven path validates the pushed tag, verifies that the tagged commit is contained in `main`, runs the same quality gates, and only then creates the GitHub Release for that existing tag.

Published tags and releases are never silently overwritten or repointed. If a requested tag or release already exists, the workflow fails rather than mutating published version history.

## Quality gates before publishing

A release is not created merely because somebody typed a version number. The workflow first runs:
...
This reuses Serviq's normal contributor and CI command surface instead of inventing a separate release-only test path.

`make security`, `make e2e`, and `make load-test` are not release gates in OPE-304 while those targets remain separate implementation concerns. They should be added to publishing only when their dedicated tickets establish the real gates.

## Release labels are bootstrapped idempotently

The release workflow ensures these labels exist:
...
Label creation is idempotent and uses GitHub's repository-scoped `GITHUB_TOKEN`. The release paths do not require a personal access token or a paid release service.

The OPE-304 reconciliation also makes label bootstrap part of every supported release entry path. This means a manual or tag-driven release does not depend on someone remembering to recreate a missing release label first.

## Pull requests now declare release impact

`.github/pull_request_template.md` asks every contributor to state whether a change is:
...
The template also asks what users will notice, whether an upgrade or migration is required, and whether the change is breaking. This moves release-impact information into review time, where it is easier to verify, instead of reconstructing it later from commit history.

`CONTRIBUTING.md` documents the same Semantic Versioning and release rules, and `docs/RELEASING.md` is the permanent operator guide for creating and verifying releases.

## Why the first release is an alpha
...
That name is deliberate. Serviq has meaningful platform-foundation work, but the complete product is not yet production-ready. The `0` major version and `alpha.1` suffix communicate that this is an early developer/platform snapshot rather than a stable customer release.

The first release was originally published from the merged release-system work and verified after publication. Reopening OPE-304 does not rewrite or repoint that historical tag. The repository policy treats published versions as immutable history. The reconciliation work strengthens the automation and restores the cumulative documentation while preserving the existing first release.

## Security and integrity rules

The release automation uses the repository-scoped `GITHUB_TOKEN` and grants write permission only where release or label operations require it. Release inputs are validated before release commands run. Releases must point to commits contained in `main`.

The workflow does not publish production credentials, customer data, or secret-bearing release assets. Repository-level immutable-release enforcement remains intentionally deferred, but Serviq already treats published tags/releases as immutable by policy. A bad published version must be corrected with a new version rather than by moving an old tag.

## What this improves

Serviq now has a durable answer to "what exact code is version X?" Clients, contributors, and future maintainers can use the Releases page as a readable version history instead of reconstructing product snapshots from dozens of commits.

Release notes remain traceable to merged pull requests, release impact is captured before merge, and every supported release path validates both repository quality and `main` ancestry before publishing.

## What OPE-304 intentionally leaves for later

OPE-304 does not:

- deploy production environments;
- publish containers to GHCR;
- generate SBOMs;
- sign artifacts;
- create provenance or attestations;
- infer major/minor/patch automatically from labels;
- introduce release branches, GitFlow, backports, or hotfix branches;
- declare Serviq stable `v1.0.0`.

Those capabilities have separate security and operational consequences and belong in dedicated reviewed tickets.

## Completion and validation rule

The reconciled OPE-304 work is complete only after the task branches are merged into `ope304`, the final `ope304` pull request passes repository CI/security checks, and the existing `v0.1.0-alpha.1` prerelease is re-verified without modifying it. GitHub #66 and Linear OPE-304 should close only after those checks are complete.


---

# V1.0.26 — Reproducible LLM gateway dependencies and Python audits

**GitHub parent issue:** #166  
**Implementation issues:** #167, #168, #169, #170  
**Integration branch:** `v1.0.26`

## Why this ticket exists

Serviq already had committed `uv.lock` files for the API and worker, but the LLM gateway still resolved dependencies directly from `pyproject.toml` during setup and vulnerability auditing. That made the gateway less reproducible than the other Python services because compatible transitive versions could change even when no Serviq source code changed.

The audit path had a second source of machine-specific behavior. Serviq requires Python 3.14, but `uvx` creates a separate tool environment for utilities such as `pip-audit`. Without explicitly selecting Python 3.14, that isolated environment could use another locally available interpreter. The repository audit that produced V1.0.26 observed exactly this class of mismatch.

V1.0.26 removes both sources of drift without changing provider adapters, model names, API schemas, database behavior, authentication, tenant boundaries, prompts, or customer-facing runtime behavior.

## The LLM gateway now owns a committed lock
...
It was generated in GitHub Actions under Python 3.14.6 from the gateway's existing dependency declarations and validated with `uv lock --check` before commit. The gateway `pyproject.toml` was deliberately not changed, so this ticket freezes the already-approved provider SDK contract rather than upgrading providers.

A committed lock freezes the complete resolved dependency graph, including transitive packages and package hashes. This gives the gateway the same dependency model already used by the API and worker.

## Lock freshness is a required preflight

The root `Makefile` now defines `dependency-lock-check`. It runs `uv lock --check` independently in:
...
Both `make setup` and `make security` depend on this target.

This is intentionally separate from frozen installation. `--frozen` tells uv to consume the existing lock without changing it. `uv lock --check` proves the lock still agrees with `pyproject.toml`. If a dependency declaration changes without a matching lock update, Serviq now fails at the dependency boundary instead of silently repairing or ignoring the mismatch.

## Setup and audit exports are frozen

All three Python services now use `uv sync --frozen` during `make setup`. The API, worker, and LLM gateway vulnerability exports all use `uv export --frozen --no-dev`.

Normal setup and security commands therefore consume reviewed lock state. They are not allowed to create or rewrite lockfiles as a side effect.

## `pip-audit` always uses Python 3.14

Every Python dependency audit now runs the equivalent of:

```text
uvx --python 3.14 --from pip-audit==2.10.1 pip-audit ...
```

The explicit `--python 3.14` controls the separate `uvx` tool environment. A machine that also has Python 3.13 installed can no longer change the scanner interpreter through local discovery order. The audit tool remains pinned at `pip-audit==2.10.1`.

V1.0.26 does not replace uv, replace pip-audit, add a dependency-management service, or alter provider SDK declarations.

## CI verifies dependency setup leaves Git clean

The CI quality job runs `make setup` and immediately checks `git status --porcelain`.

If setup generates or modifies a repository file, CI prints the dirty status and fails before lint, type checking, or tests. This catches regressions such as removing `--frozen`, forgetting to commit a lockfile, or introducing another dependency command that rewrites tracked state.

The check is intentionally placed immediately after dependency setup so a failure points directly to installation rather than later build/test output.

## Security keeps non-zero failure behavior

The Security dependency job validates all three Python locks before auditing. Each Python export is frozen and each `pip-audit` invocation explicitly selects Python 3.14.

The existing JSON report artifacts and final audit-status enforcement remain intact. A real dependency vulnerability therefore still produces a non-zero Security result. V1.0.26 does not add vulnerability ignores or convert a failing audit into a warning merely to make automation green.

## Release paths use the same dependency contract

The foundation, manual, and tag-driven release jobs continue to use the repository's shared `make setup` command. Each release path now checks the worktree immediately after dependency installation and refuses to continue if setup changed repository state.

No release tag, version rule, label behavior, token permission, or published release history changes. The existing `v0.1.0-alpha.1` release is not moved or republished.

## Contributor workflow

`CONTRIBUTING.md` now states that the API, worker, and LLM gateway require Python 3.14.x and each owns a committed `uv.lock`. Contributors can run `make dependency-lock-check` to verify lock freshness without installing dependencies.

An intentional Python dependency change must update the owning `pyproject.toml`, regenerate the matching `uv.lock` under Python 3.14, review both together, and commit them together. Hand-editing a lock merely to satisfy CI is not an accepted workflow.

## What this improves

### Reproducible development

A clean checkout has one reviewed Python dependency graph for each service. Gateway installation no longer depends on whichever compatible transitive release happens to be newest at setup time.

### Reproducible security evidence

The vulnerability scanner receives dependency exports produced from committed locks and runs under the same Python major/minor line as the application services.

### Safer releases

Release dependency preparation must leave the repository unchanged. A release cannot unknowingly proceed from dependency commands that generated or rewrote tracked state.

### Easier debugging

A stale lock fails at the lock-check step. A dirty dependency setup fails immediately after setup. These failure locations are much easier to diagnose than discovering dependency drift later during provider testing or release publication.

## Validation evidence

The LLM gateway lock bootstrap completed in GitHub Actions under Python 3.14.6. The bootstrap successfully generated the lock and successfully ran `uv lock --check` before the lock was committed.

The final branch is validated through Serviq's normal CI and Security workflows. CI covers frozen setup, the clean-worktree gate, lint, type checking, tests, Compose configuration, and existing integration jobs. Security covers CodeQL, Gitleaks, Trivy, lock validation, frozen dependency exports, explicit Python 3.14 `pip-audit`, JSON report artifacts, and non-zero audit enforcement.

The stale-lock negative case is exercised on temporary copied gateway dependency metadata so the real branch does not need to be intentionally dirtied. Editing the copied `pyproject.toml` without updating its copied lock must make `uv lock --check` return non-zero.

`make e2e` and `make load-test` remain intentional non-zero placeholders and are not claimed as passing evidence for this ticket.

## What did not change

V1.0.26 does not change:

- provider adapter contracts;
- OpenAI, Anthropic, Gemini, or OpenRouter behavior;
...
This is a developer, CI, security, and release-reliability change only.

## Existing repository-context discrepancy

During V1.0.26 we confirmed that `services/api/uv.lock` and `services/worker/uv.lock` already exist even though one sentence in `docs/repo_context.md` still says there is no committed Python lockfile.

That sentence is stale documentation. `docs/repo_context.md` is outside the approved V1.0.26 file list, so this focused ticket records the discrepancy rather than widening scope. The actual committed lockfiles and executable dependency commands are treated as implementation evidence.
...
docker compose -f infra/docker/compose.yml run \
  ...
  --service-ports \
  --no-deps \
  --volume "$PWD/infra/keycloak/serviq-test-realm.json:/opt/keycloak/data/import/serviq-realm.json:ro" \
...
# V1.3.04D — Worker-owned durable failed-upload cleanup runtime

## What problem this closes

File-backed knowledge uploads already had a durable PostgreSQL cleanup state machine,
including `prepared`, `pending`, `referenced`, `succeeded`, and `exhausted` states.
The API could perform immediate best-effort cleanup after a failed request, but the
audited system had no production process continuously replaying obligations that
survived request failure or process restart. A failed DELETE, an ambiguous object
PUT, or a process crash could therefore leave a durable cleanup record waiting with
no normal runtime caller.

V1.3.04D closes that caller gap. It does not change the public knowledge-source API
or create a second cleanup contract.

## Why the worker owns cleanup

The durable worker already owns background work that must survive API request and
process lifetimes. The cleanup scheduler therefore runs in `services/worker`, next
to outbox publication and knowledge-sync consumption.

Two tempting alternatives were rejected. Running cleanup from FastAPI lifespan
would make recovery depend on API process availability. Importing API source modules
directly into the worker would couple two independently packaged Python services.
A new Kafka cleanup topic was also unnecessary because PostgreSQL already contains
the durable source of truth and retry schedule.

ADR-028 records this ownership decision:
...
The production reconciler is:

```text
services/worker/app/jobs/knowledge_upload_cleanup.py
```

Worker composition is:

```text
services/worker/app/main.py
```

The worker's existing S3-compatible boundary now exposes only the two additional
operations cleanup needs:
...
Those methods live in:

```text
services/worker/app/core/object_storage.py
```

No object-list or arbitrary-prefix deletion capability was added.

## How one cleanup cycle works

Each poll selects only a bounded number of due `prepared` or `pending` rows. The
query uses PostgreSQL `FOR UPDATE SKIP LOCKED`, allowing more than one worker process
to share the cleanup table without deliberately claiming the same due row in the
same claim window.

For each row, the worker reconstructs the only valid raw-object key from the stored
tenant, source, and object UUIDs:

```text
tenants/{tenantId}/knowledge/{sourceId}/raw/{objectId}
```

If the persisted key does not exactly match that generated identity, the obligation
is exhausted without touching object storage. This prevents a corrupted cleanup row
from becoming an arbitrary delete primitive.

Inside the short claim transaction the worker increments the attempt counter,
changes the row to `pending`, and advances `next_attempt_at`. The transaction then
commits before any object-storage network call. A slow or unavailable storage
service therefore does not keep a PostgreSQL row lock open.

For an ambiguous PUT outcome, the worker first performs a metadata-only HEAD/exists
check. A missing object or unavailable HEAD is not treated as proof that cleanup is
complete. It consumes one bounded observation attempt. A visible object is deleted
through the idempotent storage operation. After confirmed cleanup, a new short
transaction marks the obligation `succeeded` and releases its matching tenant-scoped
`knowledge_upload_reservations` row.

After three unresolved reconciliation attempts, the obligation becomes `exhausted`.
Its reservation stays charged because Serviq still cannot prove that the possible
raw object is gone. `exhausted` is therefore operator-visible durable state rather
than silent quota forgiveness.

## Restart and concurrency behavior

The retry lease is persisted before storage I/O. If a worker crashes after claiming
an item, another worker does not need any in-memory handoff. Once the stored
`next_attempt_at` becomes due, a later process can recover the same obligation from
PostgreSQL.

Terminal `referenced`, `succeeded`, and `exhausted` rows are not selected as new due
work. A successfully referenced source is therefore never intentionally deleted by
the scheduler.

## Logging and privacy

Cleanup logs use stable event names, cleanup/tenant IDs, attempt counts, outcomes,
and aggregate batch counts. They do not include object keys, filenames, upload
bodies, bucket names, storage endpoints, credentials, or raw infrastructure
exception text. Loop-level infrastructure failure uses a fixed safe log event and
retries on the next poll.

## How it is tested

Unit coverage verifies the new storage HEAD/delete boundary, missing-object behavior,
idempotent deletion, provider failure normalization, key validation, worker loop
cancellation, process composition, and secret-safe failure logging.

The existing `Knowledge Quota Integration` workflow now also installs the worker and
runs:

```text
services/worker/tests/integration/test_knowledge_upload_cleanup.py
```

against real PostgreSQL and the repository's S3-compatible storage service. The
integration suite proves:

- a due pending cleanup deletes the synthetic orphan and releases its reservation;
- a stale ambiguous `prepared` obligation records a retry instead of false success;
- a fresh database session can recover that obligation after the persisted deadline,
  demonstrating restart recovery without process memory;
- three unresolved observations become `exhausted` while quota remains held;
- a mismatched generated object key exhausts safely without a storage delete.

Normal PR validation still requires Ruff, strict mypy, the complete test suite,
Compose validation, PostgreSQL migration/integration, object-storage integration,
live Keycloak validation, Security, Knowledge Sync, Knowledge Quota, and Outbox
Publisher workflows. V1.3.04D is not complete merely because the scheduler file
exists.

## Operational use

Routine cleanup does not require a tenant-facing endpoint or an operator to call a
function manually. Running the normal Serviq worker starts the cleanup loop with the
other durable jobs.

For diagnosis and failure-injection instructions, use:

```text
docs/KNOWLEDGE_UPLOAD_CONSISTENCY_RUNBOOK.md
```

Only trusted internal tooling should inspect or recover an exhausted obligation.
Do not accept a user/operator supplied raw object key as the destructive input.

## Rollback and compatibility

V1.3.04D adds no database migration and changes no public request/response contract.
The scheduler can be removed from worker composition without a schema rollback, but
doing so recreates the original operational gap: unresolved rows remain durable and
stop recovering automatically.

The existing API request-time cleanup remains as a fast path. The worker is the
long-lived recovery path.

## What remains intentionally unresolved

This ticket does not solve V1.3.04C pre-parser multipart admission. Large multipart
file parts can still be spooled before Serviq's file-byte/concurrency checks run.
It also does not add V1.3.07A URL raw-version byte accounting/recovery, parse-event
consumption/index handoff, automatic requeue of `exhausted` cleanup, retention purge,
or a new cleanup operations UI. Those remain separate contracts and tickets.
