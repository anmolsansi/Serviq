...
Feature code must use the OPE-301 typed raw-key helper. The original filename may be retained only as safe metadata and never participates in object-key construction.

Object-storage network I/O does not run inside a database transaction. The service authorizes first, validates the upload, stores the generated raw object, then creates the pending database row in a separate transaction. A database failure after upload triggers best-effort deletion of only the object generated by that request.
