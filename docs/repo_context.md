...
CCR-004 is now complete at migration head: revision `20260814_0003` adds `fk_memberships_created_by_invitation_id_organization_invitations` from `memberships.created_by_invitation_id` to `organization_invitations.id` with `ON DELETE SET NULL`. Its downgrade drops this FK before dropping invitation tables, restoring the intentional OPE-277 intermediate state. Later invitation runtime/API code may now depend on the final FK but must not alter the schema contract without a CCR.
