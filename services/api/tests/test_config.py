...
def test_valid_local_settings_load() -> None:
    ...
    assert settings.object_storage_bucket == "serviq-local"
    assert settings.session_secret.get_secret_value() == "local-placeholder"
    assert settings.local_webhook_allowlist == ("localhost", "127.0.0.1")


def test_unknown_environment_fails_with_field_name_only() -> None:
...
def test_malformed_url_fails_without_echoing_value() -> None:
    malformed = "not-a-valid-http-url"
    env = BASE_ENV | {"SERVIQ_PUBLIC_BASE_URL": malformed}

    with pytest.raises(SettingsError) as error:
        load_settings(env)

    assert "SERVIQ_PUBLIC_BASE_URL" in str(error.value)
...
def test_production_requires_nonempty_secrets_without_echoing_values() -> None:
    ...
    env = BASE_ENV | {
        ...
        "SESSION_SECRET": "",
        "LLM_GATEWAY_INTERNAL_TOKEN": sentinel,
    }
...
def test_tenant_provider_key_is_not_part_of_platform_settings() -> None:
    env = BASE_ENV | {"OPENAI_API_KEY": "tenant-provider-key"}
